#Attaching District Boundaries to State file 
#JOSE M HERNANDEZ
#10/17/16
library(sp)
library(rgdal)
...
MO_data <- readRDS("~/Google Drive/CRPE/KauffmanKC/Data/MO_complete.rda")
...
MO_data$district <- rep(NA, nrow(MO_data))
MO_data$district <- over(MO_data, MO1)$NAME

#write the augmented state dataset to new .dta file
schools <- as.data.frame(MO_data)
####
saveRDS(schools, "~/Google Drive/CRPE/KauffmanKC/Data/KC2010_2014_city_district.rda")
####
