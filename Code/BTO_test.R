...
## data file
data <- readRDS("~/Google Drive/CRPE/KauffmanKC/Data/MO_complete.rda")
str(data)
## years
...
data$city <- as.character(as.character(data$city))
...
master$temp <- 1
master.sec <- ddply(master, .(city.new, time, sector), function(x)
  c(enrolltotal = sum(x$enroll),
    enrollbeat  = sum(x[which(x$beat==1),]$enroll),
    schooltotal = sum(x$temp),
    schoolbeat  = sum(x[which(x$beat==1),]$temp) ) )
...
master.sec2 <- reshape(master.sec, 
                       timevar=c("sector"), 
                       idvar=c("city.new", "time"), 
                       direction="wide")

# Merge the two aggregated dataframes
master2 <- merge(master.agg, master.sec2, by=c("city.new", "time"))
...
# Reshape (long -> wide)
#bto <- reshape(master2, timevar="time", idvar="city.new", direction="wide")
#names(bto)[1] <- "city"
####################################################################

### (2) STAGNANCY MEASURE

myvars <- c("YEAR", "seasch", "city.new", "ENROLLMENT_GRADES_K_12", "subject")

stdata <- data[myvars]
names(stdata) <- c("year", "seasch", "city.new", "enroll", "subject")
...
summary(change) #23.17% stuck in year 1 
...
change2$stuck.sch.y1 <- 0.1707 
## reduce dataframe
vars <- c("city.new", "stuck.sch.y1","stuck.sch.y1all3","stuck.sch.y1all7")
...
stagnant <- merge(stag, change2, by="city.new")
names(stagnant)[names(stagnant)=="city.new"] <- "city"
...
myvars <- c("YEAR", "seasch", "city.new", "level", "subject", "ENROLLMENT_GRADES_K_12", "LUNCH_COUNT_FREE_REDUCED", 
            "ENROLLMENT_WHITE", "ENROLLMENT_BLACK", "ENROLLMENT_HISPANIC")
qdata <- data[myvars]

names(qdata) <- c("year", "seasch", "city.new", "level", "subject", "enroll", "numfrl", "numwhite", "numblack", "numhisp")
...
quint <- ddply(qdata, .(city.new, time, schlevel), function(x) 
...
vars <- c("city", "time", "schlevel", "p20", "p80", "p20.enroll",
